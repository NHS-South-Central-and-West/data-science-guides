...
What is needed is to build coherent, logically-specified scientific models from which we design our statistical analysis. @mcelreath2023 makes this case, arguing that we need to build theory-driven scientific models that describe the causal relationship we are modelling before we build 
...
Ultimately, using confidence intervals with a hypothesis test doesn't get you much further than p-values. So embracing uncertainty in the results of a hypothesis test isn't
...
You probably realise the answer by now... The next step? The next step is to stop testing null hypotheses and **start estimating meaningful quantities**. The goal for this chapter is that readers come away with an understanding of the significant limitations of null hypothesis significance testing and the alternative approaches that could be used in its place, while also learning how to give themselves the best possible shot at doing hypothesis testing well. 

I know that this methodological tool is not going away, and I hope that this chapter has helped set people up reasonably well when carrying out tests. Equally, I hope that they will consider the alternatives, perhaps gradually weaning themselves off this bad habit and moving towards the big, wide world of estimation.


<!-- #### Start With Regressions -->

<!-- Given all this, the alternative to hypothesis tests should start with regression modelling as the basis for effect size estimation. While regression modelling is far from sufficient for robust estimation, it is a better starting point than statistical tests, because the structure of regression modelling places more emphasis on the effect size. Further, estimating effect sizes does not rule out the use of statistical tests, but it does require redirecting our focus and making sure to treat the magnitude and direction of effect size as the most important part of the analysis. The premise of statistical tests is that they test for the existence of an effect, so it is inherently a little messy to try and rework them such that the null hypothesis is not the focus. Even if the test itself is designed robustly, many code outputs will still centre the null hypothesis when reporting a statistical test (this is another advantage offered by the {infer} package). -->

<!-- I think that regression modelling gives a lot more flexibility when we are attempting to estimate an effect as precisely as possible, and the process and resulting outputs of a regression model lend themselves more naturally to a focus that steers away from hypothesis testing and towards estimating effects. -->

<!-- I think it is important to stress the distinction here between the null hypothesis testing framework and the statistical tests used to carry them out. The tests themselves are not really the problem. However, they are inextricably tied to null hypothesis testing. Nowhere is this more evident than the outputs that and Base R spits out whenever you carry out a statistical test. The entire framing of the results is based on the null hypothesis and the corresponding p-values. Statistical tests can be reworked to frame them in terms less dependent on the null hypothesis and place less emphasis on p-values. They can also estimate effect sizes too. So it's not like statistical tests themselves are not useful. But I think regression models just make life easier. Ultimately, the statistical tests discussed in this chapter (and many more) are just special cases of linear models [@lindelov2019], and the results in both will be identical [@zablotski2019] (if not the outputs), so the same things can be achieved with either approach, but regression modelling offers more flexibility, allows for the layering of complexity/precision in estimates, and the way regression outputs are framed are generally a little more balanced.  -->

<!-- My impassioned advocacy for regression modelling over other approaches is, no doubt, a product of my own biases. As a social scientist by training, regression modelling was my first love, and it's the basis from which I think about virtually all statistical methods. Deep down, I would probably argue for "starting with regressions" when it comes to just about any problem. But I'd like to think the case I'm making here is defensible. Treating regressions as the first point of call for inferential statistics is not a necessary prerequisite for "doing things the right way", but doing things the right way becomes easier, because it is easier frame everything in terms of the magnitude and direction of effects. -->
