...
options(
  ggplot2.discrete.fill = c('#217CA3', '#FFC933', '#C4274C'),
  ggplot2.discrete.colour = c('#217CA3', '#FFC933', '#C4274C')
  ) 
...
We can use the `geom_histogram()` function to create a histogram of a continuous variable, and the `geom_bar()` function to create a bar chart of a discrete variable. The `binwidth` argument can be used to control the width of the bins in the histogram, and the `position` argument can be used to control how the bars are displayed.
...
Bar plots are an effective way to visualize the observed relationship (or association, at least) between a discrete explanatory variable and a discrete outcome (whether binary, ordinal, or categorical). We can use the ``geom_bar()`` function to create bar plots, and the ``fill`` argument to split the bars by a particular variable and display them in different colours.

We can use bar plots to compare heart disease prevalence across discrete groups in the data.
...
# age and heart disease, split by sex
df |>
...
# filter zero values and add regression line
df |>
  filter(resting_bp != 0) |> 
  ggplot(aes(age, resting_bp)) +
  geom_point(alpha = 0.8, size = 3, colour = 'gray30')
...
There are a wealth of resources available to help you learn more about ggplot2 and data visualisation. Here are some of the best freely available resources:
